  ...
  if (collection === "serviceRequests") {
    ...
    return {
      ...
      summary: {
        ...
        "Meal prep notes": formatValue(payload.mealPrepNotes),
        "Move prep package": formatValue(payload.movePrepPackage),
        "Move prep options": formatValue(payload.movePrepOptionSummary),
        "Move prep notes": formatValue(payload.movePrepNotes),
        "Move prep disclaimer": formatValue(payload.movePrepDisclaimer),
        "Whole Home visit type": formatValue(payload.wholeHomeVisitType),
        "Maintenance cadence": formatValue(payload.wholeHomeRecurringCadence),
        "Whole Home condition": formatValue(payload.wholeHomeCondition),
        "Whole Home add-ons": formatValue(payload.wholeHomeAddOnSummary),
        "Cleaning / reset type": formatValue(payload.areaResetCleaningType),
        "Repeat area support": formatValue(payload.areaResetRepeatSupport),
