  ...
  if (collection === "serviceRequests") {
    ...
    return {
      ...
      summary: {
        ...
        "Home size": formatValue([payload.squareFootage ? `${payload.squareFootage} sq ft` : "", payload.bedrooms, payload.bathrooms].filter(Boolean).join(", ")),
        "Whole-home cleaning type": formatValue(payload.wholeHomeCleaningType),
        "Whole-home condition": formatValue(payload.wholeHomeCondition),
        "Cleaning / reset type": formatValue(payload.areaResetCleaningType),
        "Cleaning add-ons": formatValue(payload.wholeHomeAddOnSummary || payload.areaResetAddOnSummary),
