...
function ModalServiceBadge({ item }: { item: AdminDoc }) {
  const look = getServiceLook(item);
  const compactLabel = getCompactServicePillLabel(item);
  const fullLabel = look.label;
  return (
    <span
      className={`inline-flex max-w-full items-center gap-2 rounded-full border px-3 py-1.5 text-[10px] font-black uppercase tracking-[0.08em] sm:text-[11px] ${look.badge}`}
      title={fullLabel}
    >
...
function AdminCollapsibleSection({
  ...
  return (
    <details id={id} className="group scroll-mt-28 mb-4 w-full max-w-full overflow-hidden rounded-3xl border border-[#eadfc8] bg-white shadow-sm">
      <summary className="flex cursor-pointer list-none items-center justify-between gap-3 px-4 py-3 outline-none transition hover:bg-[#fbf6ea] focus-visible:ring-4 focus-visible:ring-[#075c58]/15 sm:px-5 sm:py-4 [&::-webkit-details-marker]:hidden">
        <span className="min-w-0">
          <span className="block text-[11px] font-black uppercase tracking-[0.18em] text-[#b98a2f]">{eyebrow}</span>
          {title && <span className="mt-1 block break-words text-[15px] font-black leading-5 text-[#075c58] sm:text-base">{title}</span>}
        </span>
        <span className="inline-flex shrink-0 items-center justify-center rounded-full border border-[#d8c18f] bg-[#fbf6ea] px-3 py-1 text-[11px] font-black uppercase tracking-[0.06em] text-slate-700 shadow-sm">
          <span className="group-open:hidden">Open</span>
          <span className="hidden group-open:inline">Close</span>
        </span>
      </summary>
      <div className="border-t border-[#eadfc8] bg-white px-3 py-4 sm:px-5 sm:py-5">
        {children}
      </div>
    </details>
  );
      ...
      <div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
        {navItems.map((navItem) => (
          <a
            ...
            className="flex min-h-14 items-center gap-2 rounded-2xl border border-[#eadfc8] bg-white px-3 py-2 text-left transition hover:border-[#075c58]/35 hover:bg-[#fbf6ea] hover:shadow-sm"
          >
            <span className="inline-flex h-7 min-w-7 shrink-0 items-center justify-center rounded-full border border-[#d8c18f] bg-[#fbf6ea] px-2 text-[10px] font-black uppercase tracking-[0.08em] text-[#b98a2f] shadow-sm">
              {navItem.label}
            </span>
...
function AdminDetailSnapshot({ collectionName, item }: { collectionName: string; item: AdminDoc }) {
  ...
  return (
    <AdminCollapsibleSection id="admin-section-snapshot" eyebrow="Snapshot" title="Review the customer/request basics">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div className="min-w-0">
          <div className="flex min-w-0 flex-wrap items-center gap-2">
            <p className="text-xs font-black uppercase tracking-[0.18em] text-[#b98a2f]">Snapshot</p>
            {collectionName === "serviceRequests" && <ModalServiceBadge item={item} />}
          </div>
...
function normalizeOriginalAnswerFieldKey(key: string) {
  return key.toLowerCase().replace(/[^a-z0-9]/g, "");
}

function originalAnswerValueToText(value: unknown): string {
  ...
  if (Array.isArray(value)) {
    return value
      .map((entry) => originalAnswerValueToText(entry))
      .filter(Boolean)
      .join(", ");
  }
  ...
  if (value && typeof value === "object") {
    const entries = Object.entries(value as Record<string, unknown>)
      .map(([key, entry]) => {
        const text = originalAnswerValueToText(entry);
...
function findOriginalAnswerValueInObject(value: unknown, wantedKeys: string[], depth = 0): string {
  if (!value || typeof value !== "object" || depth > 4) return "";
  if (Array.isArray(value)) {
    for (const entry of value) {
      const nested = findOriginalAnswerValueInObject(entry, wantedKeys, depth + 1);
  ...
  const normalizedWanted = new Set(wantedKeys.map(normalizeOriginalAnswerFieldKey));

  for (const [key, entry] of Object.entries(record)) {
    if (normalizedWanted.has(normalizeOriginalAnswerFieldKey(key))) {
      const text = originalAnswerValueToText(entry);
      if (text) return text;
    }
  }
  ...
  for (const containerKey of likelyContainers) {
    ...
    if (!nested) continue;
    const text = findOriginalAnswerValueInObject(nested, wantedKeys, depth + 1);
    if (text) return text;
  }
  ...
  if (depth < 2) {
    for (const [key, entry] of Object.entries(record)) {
      ...
      const text = findOriginalAnswerValueInObject(entry, wantedKeys, depth + 1);
      if (text) return text;
    }
  }
...
function getOriginalRequestAnswer(item: AdminDoc | null, keys: string[]) {
  if (!item) return "";
...
export default function AdminTable({
  ...
  const [laundryBagsUsed, setLaundryBagsUsed] = useState("");
  const [laundryBagsReturned, setLaundryBagsReturned] = useState("");
  const [laundryBagTrackingNotes, setLaundryBagTrackingNotes] = useState("");
  const [requestNotesBusy, setRequestNotesBusy] = useState(false);
  const [requestNotesMessage, setRequestNotesMessage] = useState("");
  ...
  useEffect(() => {
    ...
    const nextServiceKey = selected ? getServiceKey(selected) : "";
    const nextRequiresReviewedCheckoutAmount = ["whole-home-reset", "specific-area-reset", "move-out-cleaning", "move-prep-home-reset"].includes(nextServiceKey);
    const nextMode = nextRequiresReviewedCheckoutAmount ? "custom" : guessCheckoutMode(selected);
    ...
    setRequestAdminNotes(selected?.adminNotes ? String(selected.adminNotes) : "");
    setLaundryBagsUsed(selected?.laundryBagsUsed !== undefined && selected?.laundryBagsUsed !== null ? String(selected.laundryBagsUsed) : "");
    setLaundryBagsReturned(selected?.laundryBagsReturned !== undefined && selected?.laundryBagsReturned !== null ? String(selected.laundryBagsReturned) : "");
    setLaundryBagTrackingNotes(selected?.laundryBagTrackingNotes ? String(selected.laundryBagTrackingNotes) : "");
    setRequestNotesMessage("");
    setRequestNotesError("");
    setRepeatLaundryDate("");
    setRepeatLaundryWindow(getOriginalRequestAnswer(selected, ["preferredWindow", "laundryPickupWindow", "preferredTime", "pickupWindow", "pickupTime", "requestedTime", "schedulingPreference"]));
    setRepeatLaundryPickupSpot(getOriginalRequestAnswer(selected, ["laundryPickupSpot", "pickupSpot", "pickupLocation", "laundryPickupLocation", "pickupInstructions"]));
    setRepeatLaundryReturnSpot(getOriginalRequestAnswer(selected, ["laundryReturnSpot", "laundryDropoffSpot", "laundryDropOffSpot", "returnSpot", "dropoffSpot", "dropOffSpot", "returnLocation", "laundryReturnLocation"]));
    setRepeatLaundryDetergent(getOriginalRequestAnswer(selected, ["laundryDetergentPreference", "laundryDetergent", "detergent", "detergentSelection", "customerProvidedDetergent", "customerProvidedDetergentSelected", "productPreference", "soapPreference"]));
    setRepeatLaundryDryerPreference(getOriginalRequestAnswer(selected, ["laundryDryerPreference", "dryPreference", "dryingPreference", "dryerPreference", "hangDryItems", "delicates"]));
    setRepeatLaundryFoldPreference(getOriginalRequestAnswer(selected, ["laundryFoldPreference", "foldingPreference", "sortingPreference", "foldPreference"]));
    setRepeatLaundryAmount(getOriginalRequestAnswer(selected, ["laundryAmount", "laundryBagEstimate", "laundryHamperEstimate", "bagEstimate", "hamperEstimate", "estimatedLaundry", "laundryTypes"]));
    setRepeatLaundrySpecialInstructions(getOriginalRequestAnswer(selected, ["laundrySpecialInstructions", "laundryNotes", "specialInstructions", "notes", "requestDetails", "laundryAccessNotes", "parkingAccess", "accessNotes"]));
    setRepeatLaundryAddOnNote(getOriginalRequestAnswer(selected, ["laundryAddOns", "addOns", "addons", "comfortersBedding", "heavyItems", "bulkyItems"]));
  ...
  const laundryBagsStillOut = Math.max(0, Math.round(toNumber(laundryBagsUsed)) - Math.round(toNumber(laundryBagsReturned)));
  ...
  async function createRepeatLaundryRequest() {
    ...
    try {
      const token = await firebaseAuth.currentUser?.getIdToken();
      const res = await fetch("/api/admin/create-repeat-laundry-request", {
        method: "POST",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
        body: JSON.stringify({
      ...
      if (!res.ok || !data.ok) {
        if (data.duplicateRequestId) {
          setRepeatLaundryCreatedRequestId(String(data.duplicateRequestId));
          setRepeatLaundryMessage(data.error || "A repeat request already exists for that source request and pickup date. Open the existing request instead.");
          return;
        }
        throw new Error(data.error || "Unable to create repeat laundry request.");
      }
  ...
  async function createFollowUpServiceRequest() {
    ...
    try {
      ...
      if (!res.ok || !data.ok) {
        if (data.duplicateRequestId) {
          setFollowUpCreatedRequestId(String(data.duplicateRequestId));
          setFollowUpMessage(data.error || "A follow-up request already exists for that source request and visit date. Open the existing request instead.");
          return;
        }
        throw new Error(data.error || "Unable to create follow-up service request.");
      }
  ...
  async function saveRequestInternalNotes() {
    if (!selected || collectionName !== "serviceRequests") return;

    setRequestNotesBusy(true);
    setActiveAction("Saving internal request notes...");
    setRequestNotesMessage("");
    setRequestNotesError("");

    try {
      const token = await firebaseAuth.currentUser?.getIdToken();
      const res = await fetch("/api/admin/update-request-notes", {
        method: "POST",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
        body: JSON.stringify({
          requestId: selected.id,
          adminNotes: requestAdminNotes,
          laundryBagsUsed: selected.service === "laundry-rescue" ? laundryBagsUsed : undefined,
          laundryBagsReturned: selected.service === "laundry-rescue" ? laundryBagsReturned : undefined,
          laundryBagTrackingNotes: selected.service === "laundry-rescue" ? laundryBagTrackingNotes : undefined,
        }),
      });

      const data = await res.json().catch(() => ({}));
      if (!res.ok || !data.ok) throw new Error(data.error || "Unable to save internal notes.");

      const updates = data.updates || {};
      setSelected((prev) => (prev ? { ...prev, ...updates } : prev));
      setItems((prev) => prev.map((item) => item.id === selected.id ? { ...item, ...updates } : item));
      setRequestNotesMessage("Internal notes saved.");
    } catch (error) {
      setRequestNotesError(error instanceof Error ? error.message : "Unable to save internal notes.");
    } finally {
      setRequestNotesBusy(false);
      setActiveAction("");
    }
  }
  ...
  const selectedServiceKey = selected ? getServiceKey(selected) : "";
  const selectedRequiresReviewedCheckoutAmount = ["whole-home-reset", "specific-area-reset", "move-out-cleaning", "move-prep-home-reset"].includes(selectedServiceKey);
      ...
      {selected && (
        <div className="fixed inset-0 z-[300] overflow-y-auto overflow-x-hidden bg-black/50 px-2 pb-4 pt-2 sm:px-4 sm:py-4">
          <div className="mx-auto min-h-[calc(100dvh-1rem)] w-full max-w-6xl overflow-hidden rounded-3xl bg-white p-3 shadow-2xl sm:min-h-0 sm:max-h-[92dvh] sm:overflow-auto sm:p-6">
            <div className="sticky top-0 z-30 -mx-3 -mt-3 mb-3 flex items-center justify-between gap-3 rounded-t-3xl border-b border-[#eadfc8] bg-white/95 px-3 py-3 backdrop-blur sm:-mx-6 sm:-mt-6 sm:mb-4 sm:px-6 sm:py-4">
              <div className="min-w-0 flex-1">
                <div className="flex min-w-0 flex-wrap items-center gap-2">
                  <p className="text-[11px] font-bold uppercase tracking-[0.18em] text-[#b98a2f]">Admin details</p>
                  {collectionName === "serviceRequests" && <ModalServiceBadge item={selected} />}
                </div>
                <h3 className="mt-1 truncate text-xl font-bold text-[#075c58] sm:text-2xl">{getRecordDisplayName(selected)}</h3>
              ...
              <div className="flex shrink-0 flex-wrap items-center justify-end gap-2">
                <button onClick={() => setSelected(null)} className={`${getAdminActionClass("quiet")} !min-h-10 !px-4 !py-2 text-xs sm:text-sm`}>Close</button>
              </div>
            </div>
            ...
            {showApplicationOnboardingPanel && (
              <AdminCollapsibleSection id="admin-section-applicant-email" eyebrow="Applicant email" title="Send applicant communication">
            ...
            {showApplicationOnboardingPanel && (
              <AdminCollapsibleSection id="admin-section-applicant-onboarding" eyebrow="Applicant onboarding" title="Track helper/partner onboarding">
                <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                  <div className="max-w-3xl">
            ...
            {showCommercialQuotePanel && (
              <AdminCollapsibleSection id="admin-section-commercial-quote" eyebrow="Commercial quote" title="Build and send commercial quote details">
                <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                  <div className="max-w-3xl">
            ...
            {showFamilyPaymentBreakdownPanel && (
              <AdminCollapsibleSection id="admin-section-family-breakdown" eyebrow="Draft estimate builder" title="Build the customer payment summary">
            ...
            {collectionName === "serviceRequests" && (
              <AdminCollapsibleSection id="admin-section-internal-notes" eyebrow="Internal notes" title="Private request tracking">
                <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                  <div>
            ...
            {collectionName === "serviceRequests" && selected.service === "laundry-rescue" && (
              <AdminCollapsibleSection id="admin-section-repeat-laundry" eyebrow="Repeat laundry" title="Create the next Laundry Rescue pickup">
                <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                  <div>
            ...
            {collectionName === "serviceRequests" && selected.service !== "laundry-rescue" && (
              <AdminCollapsibleSection id="admin-section-recurring-service" eyebrow="Follow-up / recurring" title="Create a new follow-up request">
                <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                  <div>
            ...
            {showPaymentActions && (
              <AdminCollapsibleSection id="admin-section-payment" eyebrow="Payment / smart checkout" title="Create invoice or smart checkout">
                ...
                <div className="mt-4 rounded-3xl border border-[#d8c18f] bg-white p-4">
                  ...
                  {!selectedIsCommercial && selected.service !== "laundry-rescue" && (
                    <div className="mt-4 grid gap-2 sm:max-w-sm">
                      ...
                      >
                        {!selectedRequiresReviewedCheckoutAmount && <option value="standard">Standard package price</option>}
                        <option value="custom">{selectedRequiresReviewedCheckoutAmount ? "Reviewed/custom amount" : "Custom reviewed amount"}</option>
                      </select>
                    </div>
                  )}
                      ...
                      <div className="mt-4 grid gap-3 md:grid-cols-[0.7fr_1.3fr]">
                        ...
                        <label className="grid gap-2 text-sm font-bold text-slate-700">
                          ...
                          <input
                            ...
                            placeholder={getServiceKey(selected) === "laundry-rescue" ? "Laundry Rescue custom deposit" : getServiceKey(selected) === "commercial-reset" ? "Commercial Reset approved quote" : getServiceKey(selected) === "move-out-cleaning" ? "Move-In / Move-Out Cleaning reviewed checkout" : getServiceKey(selected) === "specific-area-reset" ? "Specific Area(s) Reset reviewed checkout" : getServiceKey(selected) === "whole-home-reset" ? "Whole Home Cleaning reviewed checkout" : getServiceKey(selected) === "family-reset-3hr" ? "Parent Reset Plan checkout" : "Custom checkout"}
                            className="rounded-2xl border border-[#eadfc8] bg-white px-4 py-3 text-sm outline-none focus:border-[#075c58]"
                          />
                        </label>
                      </div>
            ...
            {showPaymentActions && (
              <AdminCollapsibleSection id="admin-section-additional-payment" eyebrow="Additional payment" title="Create add-on or balance payment">
                <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                  <div className="max-w-2xl">
            ...
            {showCustomerStatusActions && (
              <AdminCollapsibleSection id="admin-section-status" eyebrow="Status + customer update" title="Update status or notify customer">
                <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                  <div className="max-w-2xl">
            ...
            {showFamilyReferralPanel && (
              <AdminCollapsibleSection id="admin-section-referrals" eyebrow="Family referrals" title="Generate and track referral links">
                <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                  <div className="max-w-3xl">
            ...
            {showReviewRequestPanel && (
              <AdminCollapsibleSection id="admin-section-review-request" eyebrow="Review request" title="Ask for a customer review">
                <ReviewRequestPanel
                  selected={selected}
            ...
            <AdminCollapsibleSection id="admin-section-uploads" eyebrow="Photos, docs + full answers" title="Review uploads and advanced saved answers">
              <div className="grid gap-3 sm:grid-cols-2">
